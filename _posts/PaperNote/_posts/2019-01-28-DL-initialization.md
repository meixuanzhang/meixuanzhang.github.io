...
以feedforward nerual network为例，下图展示了计算参数梯度时(来源：台大深度学习)影响因素，从最后结果可以看出对于$$l$$层参数梯度计算，其受上一层($$l-1$$层)神经元的输出$$a^{l-1}$$,当前层激活函数对其输入微分$$\sigma ' (z^l)$$,下一层($$l+1$$层)参数$$W^{l+1}$$,损失函数对下一层激活函数输入微分$$\delta^{l+1}$$影响  
...
$$
Var[\frac{\partial Cost}{\partial s^i}]=Var[\frac{\partial Cost}{\partial s^d}]\prod_{i'=i+1}^{d}n_{i'+1}Var[W^{i'}]
$$
