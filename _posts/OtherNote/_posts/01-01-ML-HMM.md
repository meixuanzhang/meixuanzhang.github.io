...
=\sum_{i=1}^N\sum_{j=1}^Na_{t}(i)a_{ij}b_{j}(o_{t+1})\beta_{t+1}(j),t=1,2,..T-1
$$  
...
$$
a_{ij}=\frac{\sum_{s=1}^S \sum_{t=2}^T   P(i_{t-1}^{(s)}=i,i_{t}^{(s)}=j \mid O^{(s)};\lambda^{(i)})  }{{t=2}^T P(i_{t-1}^{(s)}=i\mid O^{(s)};\lambda^{(i)})}\\
