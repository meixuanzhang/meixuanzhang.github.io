...
\pi_{i}=\sum_{s=1}^S \frac{P(i_{1}^{(s)}=r,O;\lambda^{(i)})}{\lambda_{\pi}}
$$  

需要对$$\pi_{i}$$进行标准化：  

$$
\pi=\frac{\sum_{s=1}^S  P(i_{1}^{(s)}=i,O;\lambda^{(i)}) } {\sum_{r=1}^N\sum_{s=1}^S  P(i_{1}^{(s)}=r,O;\lambda^{(i)})}
$$
