...
$$Q=\sum_{I_{\ast}}P(I^{\ast}\mid O^{\ast};\lambda_{(i)})logP(O^{\ast},I_{\ast};\lambda)$$

$$
P(I_{\ast}, O^{\ast};\lambda_{(i)})=P(I_{\ast}\mid O^{\ast};\lambda_{(i)})P(O^{\ast};\lambda_{(i)})\\
P(I_{\ast}\mid O^{\ast};\lambda_{(i)}) \propto P(I_{\ast}, O^{\ast};\lambda_{(i)}) \\
\mathop{\arg\max}_{\lambda} \sum\limits_{I_{\ast}}P(I_{\ast}\mid O^{\ast};\lambda_{(i)})logP(O^{\ast},I_{\ast};\lambda)\\
=\mathop{\arg\max}_{\lambda} \sum\limits_{I_{\ast}}P(I_{\ast},O^{\ast};\lambda_{(i)})logP(O^{\ast},I_{\ast};\lambda)
$$  

$$\propto$$成正比符号，$$P(O^{\ast};\lambda_{(i)})$$是常数。
