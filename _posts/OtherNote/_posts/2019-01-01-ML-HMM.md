...
已知：有S个长度相同的观测序列$$O_{\ast}=\{O_{1},O_{2},..O_{s}\}$$
未知：有S个长度相同的隐藏状态序列$$I_{\ast}=\{I_{1},I_{2}..I_{s}\}$$
求：模型参数$$\lambda$$
**明确隐变量，写出完全数据的对数似然函数**  
$$Q=\sum_{I_{\ast}}P(I_{\ast}\mid O_{\ast};\lambda_{(i)})logP(O_{\ast},I_{\ast};\lambda)$$

$$
P(I_{\ast}, O_{\ast};\lambda_{(i)})=P(I_{\ast}\mid O_{\ast};\lambda_{(i)})P(O_{\ast};\lambda_{(i)})\\
P(I_{\ast}\mid O_{\ast};\lambda_{(i)}) \propto P(I_{\ast}, O_{\ast};\lambda_{(i)}) \\
\mathop{\arg\max}_{\lambda} \sum\limits_{I_{\ast}}P(I_{\ast}\mid O_{\ast};\lambda_{(i)})logP(O_{\ast},I_{\ast};\lambda)\\
=\mathop{\arg\max}_{\lambda} \sum\limits_{I_{\ast}}P(I_{\ast},O_{\ast};\lambda_{(i)})logP(O_{\ast},I_{\ast};\lambda)
$$  

$$\propto$$成正比符号，$$P(O_{\ast};\lambda_{(i)})$$是常数。
