...
$$
\gamma_{i}=P(O,i_{t}=\frac{ P(O,i_{t}=q_{i};\lambda)}{\sum_{j=1}^N P(O,i_{t}=q_{j};\lambda)}\\
=\frac{ P(o_{1},o_{2}...o_{T},i_{t}=q_{i};\lambda)}{\sum_{j=1}^N P(o_{1},o_{2}...o_{T},i_{t}=q_{j};\lambda)}
$$

$$
P(o_{1},o_{2}...o_{T},i_{t}=q_{j};\lambda)=\sum_{r=1}^NP(o_{1},o_{2}...o_{T},i_{t+1}=q_{r},i_{t}=q_{j};\lambda)\\
=
$$
### 维特比算法  
