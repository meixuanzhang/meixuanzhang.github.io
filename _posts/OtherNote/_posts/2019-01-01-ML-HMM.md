...
对$$\pi_{i},bj{k},a_{ij}$$求导：  

因为$$\sum_{i=1}^N\pi_{i}=1,\sum_{j=1}^Na_{ij}=1,\sum_{k=1}^Mb_{j}(k)=1$$, 带有等约束条件的优化问题，使用拉格朗日乘子法，所以：  

$$
L=Q-\lambda_{\pi}(\sum_{i=1}^N\pi_{i}-1)-\lambda_{a}(\sum_{j=1}^Na_{ij}-1)-\lambda_{b}(\sum_{k=1}^Mb_{j}(k)-1)\\
$$

$$
\frac{dL}{d\pi_{i}}=\frac{d}{d\pi_{i}}(\sum\limits_{I} \sum_{s=1}^Slog\pi_{i_{1}}^{(s)}P(I,O;\lambda^{(i)}))-\lambda_{\pi}=0\\
=\frac{d}{d\pi_{i}}(\sum\limits_{I} \sum_{s=1}^Slog\pi_{i_{1}}^{(s)}P(I,O;\lambda^{(i)}))-\lambda_{\pi}=0
$$
