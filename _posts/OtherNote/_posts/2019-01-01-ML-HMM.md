...
$$Q=\sum_{I}P(I\mid O;\lambda^{(i)})logP(O,I;\lambda)$$
...
$$
\lambda^{(i+1) }=\mathop{\arg\max}_{\lambda} \sum\limits_{I}P(I\mid O;\lambda^{(i)})logP(O,I;\lambda)\\
=\mathop{\arg\max}_{\lambda} \sum\limits_{I}P(I,O;\lambda^{(i)})logP(O,I;\lambda)\\
Q=\sum\limits_{I}P(I,O;\lambda^{(i)})logP(O,I;\lambda)
$$  

**EM算法的E步，确定目标函数Q**  

$$
P(O,I;\lambda)=\prod_{s=1}^S[\pi_{i_{1}}^{(s)} b_{i_{1}}^{(s)}   (o_{1}^{(s)})\prod_{t=2}^T(a_{i_{t-1}i_{t}}^{(s)}  b_{i_{t}}^{(s)}(o_{t}^{(s)}))]\\
logP(O,I;\lambda)=\sum_{s=1}^S[log\pi_{i_{1}}^{(s)}+  \sum_{t=2}^T log a_{i_{t-1}i_{t}}^{(s)} \sum_{t=1}^T log b_{i_{t}}^{(s)}(o_{t}^{(s)})]\\

Q=\sum\limits_{I} \sum_{s=1}^S[P(I,O;\lambda^{(i)})log\pi_{i_{1}}^{(s)}+  \sum_{t=2}^T P(I,O;\lambda^{(i)})log a_{i_{t-1}i_{t}}^{(s)} \sum_{t=1}^T P(I,O;\lambda^{(i)})log b_{i_{t}}^{(s)}(o_{t}^{(s)})]
$$  
...
$$
L=Q-\lambad_{\pi}(\sum_{i=1}^N\pi_{i}-1)-\lambad_{a}(\sum_{j=1}^Na_{ij}-1)-\lambad_{b}(\sum_{k=1}^Mb_{j}(k)-1)\\
$$

$$
\frac{dL}{d\pi_{i}}=\frac{d}{d\pi_{i}}(\sum\limits_{I} \sum_{s=1}^SP(I,O;\lambda^{(i)})log\pi_{i_{1}}^{(s)})-\lambad_{\pi}=0\\
=\frac{d}{d\pi_{i}}(\sum\limits_{I} \sum_{s=1}^SP(I,O;\lambda^{(i)})log\pi_{i_{1}}^{(s)})-\lambad_{\pi}=0
$$
