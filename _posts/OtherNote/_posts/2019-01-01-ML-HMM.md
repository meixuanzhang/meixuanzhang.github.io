...
已知：有S个长度相同的观测序列$$O^{\ast}=\{O_{1},O_{2},..O_{s}\}$$  
未知：有S个长度相同的隐藏状态序列$$I^{\ast}=\{I_{1},I_{2}..I_{s}\}$$  
求：模型参数$$\lambda$$  

**明确隐变量，写出完全数据的对数似然函数**   

$$Q=\sum_{I_{\ast}}P(I^{\ast}\mid O^{\ast};\lambda^{(i)})logP(O^{\ast},I_{\ast};\lambda)$$

$$
P(I_{\ast}, O^{\ast};\lambda^{(i)})=P(I_{\ast}\mid O^{\ast};\lambda^{(i)})P(O^{\ast};\lambda^{(i)})\\
P(I_{\ast}\mid O^{\ast};\lambda^{(i)}) \propto P(I_{\ast}, O^{\ast};\lambda^{(i)}) \\
\mathop{\arg\max}_{\lambda} \sum\limits_{I_{\ast}}P(I_{\ast}\mid O^{\ast};\lambda^{(i)})logP(O^{\ast},I_{\ast};\lambda)\\
=\mathop{\arg\max}_{\lambda} \sum\limits_{I_{\ast}}P(I_{\ast},O^{\ast};\lambda^{(i)})logP(O^{\ast},I_{\ast};\lambda)
$$  
