...
若状态 j 是常返的，则$$\mu_{jj}<\infty$$时我们称j是positive recurrent，而$$\mu_{jj}=\infty$$时我们称j是null recurrent，我们令:  

$$\mu_{i}=$$\lim\limits_{n\to \infty }P_{ij}^{n}=1 \setminus \mu_{ij}$$  $$  

则,当$$\mu_{j}>0$$则状态 j  是positive recurrent, 当$$\mu_{j}=0$$则状态 j 是null recurrent
