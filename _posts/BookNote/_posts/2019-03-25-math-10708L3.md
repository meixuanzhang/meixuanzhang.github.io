...
需要证明： $$X,Y,Z$$在$$\chi$$上三个互不相交的子集，在$$g$$上$$Z$$分离了$$X$$和$$Y$$。$$P\models (X\bot Y\midZ)$$   

首先，我们观察到$$X$$到$$Y$$没有直接连接边。假设$$(X,Y,Z)$$是$$\chi$$上的分区，任何在$$g$$上的团可以完全通过$$X\cup Z$$ or $$Y\cup ZZ$$,$$I_{X}$$表示包含在$$X\cup Z$$一组团,$$I_{Y}$$表示包含在$$Y\cup Z$$一组团,已知：  

$$
P(X_{1},..X_{n})=\frac{1}{Z}\prod_{i\in I_{X}}\phi_{i}(D_{i})\cdot \prod_{i\in I_{Y}}\phi_{i}(D_{i})
$$

第一个乘法项都不包含后者的变量。因此，我们可以将乘法改写为： 

$$P(X_{1},..X_{n})=\frac{1}{Z}f(X,Z)g(Y,Z)$$  

我们观察到独立性如下：  

$$X\cup Y\cup Z$$是$$\chi$$的严格子集。让$$U=\chi - (X\cup Y\cup Z)$$.将$$U$$分割成互不相交的$$U_{1}$$和$$U_{2}$$,$$Z$$在$$g$$分离了$$X\cup U_{1}$$和$$Y\cup U_{2}$$.利用我们在分割案例中的论点，我们得到了$$((X\cup U_{1})\bot (Y\cup U_{2}))\mid Z$$应用概率的分解性质我们得到$$P\models(X\bot Y \midZ)$$
