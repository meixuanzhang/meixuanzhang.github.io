...
4、为了更准确估计赢的概率，将贪婪移动前的状态$$s$$的值更新为更接近移动后状态$$s^'$$的值，即$$V(s) \gets V(s)+\alpha[V(s'-V(s))],\alpha$$是步长参数，影响学习率。这种更新方式是一种temporal-difference 学习方法。     

**Summary**   

强化学习是一种计算方法，理解和自动以目标导向的学习和决策。 它通过代理人从与环境的直接互动中学习，而不依赖于模范监督或完整的环境模型。    
