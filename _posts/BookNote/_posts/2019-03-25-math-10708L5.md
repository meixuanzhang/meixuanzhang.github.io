...
$$X$$和$$Y$$是可观测的，与线性回归和(判别)线性分类模型一样，我们专注于$$X$$和$4Y$$之间的条件关系   
...
这样的约束仅给出粗略的指导，但是，通常对于任何给定的分布，都有许多可能的响应函数选择。 但是，有一个特殊的响应函数，即规范响应函数(canonical response function)，它是唯一与给定的指数族分布存在相关？，并且具有一些吸引人的数学特性。假设$$\xi = \eta$$,则$$f(\cdot)=\psi^{-1}(\cdot)$$,则$$f$$是canonical response function，注意一旦选择了指数族密度则$$\psi(\cdot)$$是确定的。因此，如果我们决定使用规范响应函数，指数族密度的选择完全决定了GLIM.  
...
伯努利(Bernoulli)或多项式(multinomial)分布
