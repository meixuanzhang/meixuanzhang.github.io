...
从上面我们可以看到$$p(x\mid \theta)$$，$$X$$表示随机变量，$$T(X)$$是统计量(statistic)，假设$$X$$分布取决于$$\theta$$，如果已知$$T(X)$$,$$X$$中没有关于$$\theta$$的信息,$$T(X)$$对$$\theta$$是充足的(sufficient)，就是说，观察到$$T(X)$$之后，为了推论$$\theta$$，我们可以扔掉$$X$$。 

这可以用贝叶斯论(Bayesian view)和频率论(Frequentist view)来说明。    

贝叶斯论： 

$$\theta$$ 是一个随机变量，如果满足$$\theta \bot \mid T(X)$$
