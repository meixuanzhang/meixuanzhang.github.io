...
从上面我们可以看到$$p(x\mid \theta)$$，$$X$$表示随机变量，$$T(X)$$是统计量(statistic)，假设$$X$$分布取决于$$\theta$$，如果在$$T(X)$$条件中,$$X$$没有关于$$\theta$$的信息,$$T(X)$$对$$\theta$$是充足的(sufficient)，就是说，观察到$$T(X)$$之后，为了推论$$\theta$$，我们可以扔掉$$X$$。 
...
(在频率统计中，观察到的数据被认为是随机的；如果你收集到更多的观察数据，它们将根据潜在的分布(即总体分布)而有所不同。然而，模型的参数被认为是固定的。)  
