...
对于weighted importance sampling，如果单个return中(也就是s只出现了一次,其只有一个观测G)分子和分母的$$\rho_{t:T(t)-1}$$相互抵消，那么估计值会等于这个观测值，估计的state value更可能是$$V_{b}(s)$$而不是$$V_{\pi}(s)$$,而rdinary importance sampling则不存在这个问题.   
