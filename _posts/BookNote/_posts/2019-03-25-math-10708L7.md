...
$$m ( \mathbf { x }_ {v} )$$表示结构$$\mathbf { x }_ {v}$$在数据中观测到的次数，当$$\mathbf { x } _ {v,n}=\mathbf { x }_ {v}$$时，$$\delta \left( \mathbf { x } , \mathbf { x } _ { n } \right)$$为1，其他时候为0
...
=\sum _ { c } \sum _ { \mathbf { x } _ { c } } (\sum _ { \mathbf { x }- \mathbf { x } _ { c } } m ( \mathbf { x } )) \log \left( \psi _ { c } \left( \mathbf { x } _ { c } \right) \right)-N \log Z \\
