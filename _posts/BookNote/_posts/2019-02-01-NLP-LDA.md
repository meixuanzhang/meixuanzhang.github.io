...
=\sum_{H}Q(H;\gamma,\pi)[logP(W\mid Z)+logP(Z\mid \varphi)+logP(\theta;\mathbf{α})+logP(\varphi;\mathbf{β})]\\
-\sum_{H}Q(H;\gamma,\pi)log Q(H;\gamma,\pi)\\
=E_{H}[log P(W;Z,\varphi)]+E_{H}[logP(Z;\theta)]+E_{H}[logP(\theta;\mathbf{α})]+E_{H}[logP(\varphi;\mathbf{β})]-E_{H}[logQ(Z;\theta)]-E_{H}[logQ(\theta;\gamma)]-E_{H}[logQ(\varphi;\pi)]
$$
...
=E_{H}[log(P(\theta^1;\mathbf{α^1})+logP(\theta^2;\mathbf{α^2})+...+logP(\theta^M;\mathbf{α^M})]\\
$$
