...
$$n_{d,(\cdot)}^i :$$第d篇文章，是字词中类别属于k的字数
$$
...
= \prod_{d=1}^M  \frac {\Gamma(\sum_{k=1}^K \alpha_{k})}  {\prod_{k=1}^{K}\Gamma(\alpha_{k})}  \frac {\prod_{k=1}^K \Gamma(n_{d,(\cdot)}^k + \alpha_{k})}  {\Gamma(\sum_{k=1}^{K}n_{d,(\cdot)}^k+\alpha_{k})} * \prod_{k=1}^K  \frac {\Gamma(\sum_{v=1}^V \beta_{v})}  {\prod_{v=1}^{V}\Gamma(\beta_{v})}  \frac {\prod_{v=1}^V \Gamma(n_{(\cdot),v}^k + \beta_{v})}  {\Gamma(\sum_{k=1}^{K}n_{(\cdot),v}^i+\beta_{v})}
$$
