...
=log\sum_{H}Q(H)\frac{P(W \mid H;\mathbf{α},\mathbf{β})P(H;\mathbf{α},\mathbf{β})}{Q(H;\gamma,\pi)}\\
$$
...
$$
logP(W)=D_{KL}(Q(H;\gamma,\pi) \parallel P(H\mid W))-\sum_{H}Q(H)log\frac{Q(H)}{P(H,W)}\\
=D_{KL}(Q(H;\gamma,\pi) \parallel P(H\mid W))+\sum_{H}Q(H;\gamma,\pi)log(\frac{Q(H;\gamma,\pi)}{P(H,W)})^{-1}\\
=D_{KL}(Q(H;\gamma,\pi) \parallel P(H\mid W))+\sum_{H}Q(H;\gamma,\pi)log(\frac{P(H,W)}{Q(H;\gamma,\pi)})
=D_{KL}(Q(H;\gamma,\pi) \parallel P(H\mid W))+L
$$

因为$$D_{KL}(Q(H;\gamma,\pi) \parallel P(H\mid W))\ge 0$$,所以$$L$$是$$logP(W)$$下限，对于事实$$P(W)$$是不变的，因此增大$$L$$，同时会减少$$D_{KL},P(H \mid W)$$和$$Q(H)$$越相似。
...
=E_{Q}[log P(W;Z,\varphi)]+E_{Q}[logP(Z;\theta)]+E_{Q}[logP(\theta;\mathbf{α})]+E_{Q}[logP(\varphi;\mathbf{β})]-\sum_{H}Q(H;\gamma,\pi)log Q(H;\gamma,\pi)
$$
