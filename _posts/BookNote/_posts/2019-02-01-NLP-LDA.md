...
$$
P(Z_{(m,n)}=k, \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β}) \propto \frac{n_{m,(\cdot)}^{k,-(m,n)}+\alpha_{k}}{\sum_{i=1}^K n_{m,(\cdot)}^{i,-(m,n)}+\alpha_{i}}   \frac{n_{(\cdot),(m,n)}^{k,-(m,n)}+\beta_{(m,n)}}{\sum_{v=1}^V n_{(\cdot),v}^{k,-(m,n)}+\beta_{v}}  
$$
