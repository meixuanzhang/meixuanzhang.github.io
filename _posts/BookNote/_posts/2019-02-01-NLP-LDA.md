...
$$L=\sum\limits_{H}P(W\mid H;\mathbf{α^{(i)}},\mathbf{β^{(i)}}) logP(W, H;\mathbf{α},\mathbf{β})$$

上式中无法确定$$P(W\mid H;\mathbf{α^{(i)}},\mathbf{β^{(i)}})$$   
...
通过$$D_{KL}(Q(H;\gamma,\pi,\phi) \parallel P(H\mid W;\mathbf{α},\mathbf{β}))$$相对熵来衡量分布相似，找到$$Q(H;\gamma,\pi,\phi)$$,确定目标函数:  
...
因为$$D_{KL}(Q(H;\gamma,\pi,\phi) \parallel P(H\mid W;\mathbf{α},\mathbf{β}))\ge 0$$,所以$$L_{DL}$$是$$logP(W)$$下限，对于事实$$P(W)$$是不变的，因此增大$$L_{DL}$$，同时会减少$$D_{KL},P(H \mid W)$$和$$Q(H)$$越相似。
...
最后求得$$\varphi_{k,v}$$每个类型的字词频数分布,以及文章的类别分布的参数$$\gamma_{d,k}$$    
...
$$
E_{H}[log\theta_{d,k}] \propto \Psi(\gamma_{d,k})\\
$$  
...
Notation:  
$$Z_{(m,n)}:$$第m篇文章的第n个字的类别  
$$\mathbf{Z_{-(m,n)}}:$$第m篇文章，除了第n个字外其他字的类别，是向量   
$$n_{d,(\cdot)}^k :$$第d篇文章，字词类别属于k的字词数  
$$n_{m,(\cdot)}^{k,-(m,n)}:$$第m篇文章，不包第n个字，字词类别属于k的字词数   
$$n_{(\cdot),(m,n)}^{k,-(m,n)}:$$第m篇文章，第n个字，这个字在其他文章（不包括文章m）中属于类别k类的频数   
...
$$
P(Z_{(m,n)}\mid \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β})=\frac{P(Z_{(m,n)}, \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β})}{ \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β})}\\
$$

$$
P(Z_{(m,n)}=k\mid \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β})=\frac{P(Z_{(m,n)}=k, \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β})}{ \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β})} \propto P(Z_{(m,n)}=k, \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β})\\
$$
...
$$
P(Z_{(m,n)}=k, \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β}) \propto \frac{n_{m,(\cdot)}^{k,-(m,n)}+\alpha_{k}}{\sum_{k=1}^K n_{m,(\cdot)}^{k,-(m,n)}+\alpha_{k}}   \frac{n_{(\cdot),(m,n)}^{k,-(m,n)}+\beta_{(m,n)}}{\sum_{v=1}^V n_{(\cdot),v}^{k,-(m,n)}+\beta_{v}}  
$$
...
$$
\theta_{d,k}=\frac{\sum{t=1}^{N_{d}} \parallel z_{d,t}==k \parallel }{N_{k}}\\
\varphi_{k,v} = frac {\sum_{d=1}^{M} \parallel \theta_{d}==k \parallel \parallel w_{d,t}==v\parallel }{\sum_{d=1}^{M} \parallel \theta_{d}==k \parallel N_{d}}  
$$   
