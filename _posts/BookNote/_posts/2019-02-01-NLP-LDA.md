...
+ 按第i篇文章的类别分布，随机选择一个类别作为文章i的类别，标记为$$z_{dj}\sim Multinomial(\theta_{d})$$   
...
$$
L= E_{H}log(C(H))\\
=\sum_{H}Q(H)log\frac{P(W \mid H;\mathbf{α},\mathbf{β})P(H;\mathbf{α},\mathbf{β})}{Q(H)}\\
...
$$
L= \sum_{H}Q(H)log\frac{P(W,H;\mathbf{α},\mathbf{β})}{Q(H)}\\
=\sum_{H}Q(H)log P(W,H;\mathbf{α},\mathbf{β})-\sum_{H}Q(H)log Q(H)\\
=\sum_{H}Q(H)log P(W,Z,\theta,\varphi;\mathbf{α},\mathbf{β})-\sum_{H}Q(H)log Q(H)\\
=\sum_{H}Q(H)log [P(W\mid Z)P(Z\mid \varphi)P(\theta;\mathbf{α})P(\varphi;\mathbf{β})]-\sum_{H}Q(H)log Q(H)\\
=\sum_{H}Q(H)[logP(W\mid Z)+logP(Z\mid \varphi)+logP(\theta;\mathbf{α})+logP(\varphi;\mathbf{β})]-\sum_{H}Q(H)log Q(H)\\
=E_{Q}[log P(W;Z,\varphi)]+E_{Q}[logP(Z;\theta)]+E_{Q}[logP(\theta;\mathbf{α})]+E_{Q}[logP(\varphi;\mathbf{β})]-\sum_{H}Q(H)log Q(H)
$$


$$
E_{Q}[logP(\theta;\mathbf{α})]=E_{Q}[logP(\theta^1,\theta^2...\theta^M;\mathbf{α^1},\mathbf{α^2}...\mathbf{α^M})]\\
=E_{Q}[log(P(\theta^1;\mathbf{α^1})P(\theta^2;\mathbf{α^2})...P(\theta^M;\mathbf{α^M}))]\\
=E_{Q}[log(P(\theta^1;\mathbf{α^1})+logP(\theta^2;\mathbf{α^2})+...+logP(\theta^M;\mathbf{α^M})]\\
$$

$$
E_{Q}[logP(\varphi;\mathbf{β})]=E_{Q}[logP(\varphi^1,\varphi^2...\varphi^K;\mathbf{β^1},\mathbf{β^2}...\mathbf{β^K})]\\
=E_{Q}[log(P(\varphi^1;\mathbf{β^1})P(\varphi^2;\mathbf{β^2})...P(\varphi^K;\mathbf{β^K}))]\\
=E_{Q}[logP(\varphi^1;\mathbf{β^1})+logP(\varphi^2;\mathbf{β^2})+...+logP(\varphi^K;\mathbf{β^K})]
$$ 

$$
E_{Q}[logP(Z;\theta)]=E_{Q}[logP(Z^1,Z^2...Z^M;\theta^1,\theta^2...\theta^M)]\\
=E_{Q}[log(P(Z^1;\theta^1)P(,Z^2;\theta^2)... P(Z^M;\theta^M))]\\
=E_{Q}[logP(Z^1;\theta^1)+logP(,Z^2;\theta^2)+...+logP(Z^M;\theta^M)]\\

$$   

$$
E_{Q}[log P(W;Z,\varphi)]=E_{Q}[log P(W^1,W^2...W^M;Z^1,Z^2...Z^M,\varphi^1,\varphi^2...\varphi^K)]\\
=E_{Q}[log(P(W^1;\varphi_{Z^1})P(W^2;\varphi_{Z^2})...P(W^M;\varphi_{Z^M}))]\\
=E_{Q}[logP(W^1;\varphi_{Z^1})+logP(W^2;\varphi_{Z^2})+...+logP(W^M;\varphi_{Z^M})]
$$  
