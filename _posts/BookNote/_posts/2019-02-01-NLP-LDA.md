...
=\sum_{H}Q(H;\gamma,\pi,\phi)log [P(W\mid Z)P(Z\mid \varphi)P(\theta;\mathbf{α})P(\varphi;\mathbf{β})]-\sum_{H}Q(H;\gamma,\pi,\phi)log Q(H;\gamma,\pi,\phi)\\
=\sum_{H}Q(H;\gamma,\pi,\phi)[logP(W; Z,\varphi)+logP(Z; \theta)+logP(\theta;\mathbf{α})+logP(\varphi;\mathbf{β})]-\sum_{H}Q(H;\gamma,\pi,\phi)log Q(H;\gamma,\pi,\phi)\\
