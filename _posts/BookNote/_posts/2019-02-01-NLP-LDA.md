...
$$\theta_{d}\sim Dir(\mathbf{α^d}),d\in{1,...,M}$$    

$$\theta_{d}$$是Dir分布的变量，$$\mathbf{α}$$是参数,每一篇文章会有自己参数
...
$$\varphi_{k}$$是Dir分布的变量，$$\mathbf{β^k}$$是参数,,每一类文章会有自己参数
...
+ 按$$z_{ij}$$类词汇词频分布，随机选择一个字作为第d篇文章，第j个位置的词，标记为$$w_{ij}$$
...
\varphi=(\varphi^1,\varphi^2...\varphi^M)
$$
...
=E_{Q}[log P(W\mid Z)]+E_{Q}[P(Z\mid \varphi)]+E_{Q}[P(\theta;\mathbf{α})]+E_{Q}[P(\varphi;\mathbf{β})]-\sum_{H}Q(H)log Q(H)
$$


$$
E_{Q}[P(\theta;\mathbf{α})]=
$$
