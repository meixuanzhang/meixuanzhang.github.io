...
$$
\frac{dL}{d\varphi_{k,v}}=\sum_{t=1}^N \sum_{k=1}^K \sum_{v=1}^V\pi_{d,t,k}[W_{d,t}==v] \frac{1}{\varphi_{k,v}}+\lambda_{k}
$$
...
最后求得$$\varphi_{k}$$每个类型的字词频数分布,以及文章的类别分布的参数$$\gamma_{d,k}$$    
