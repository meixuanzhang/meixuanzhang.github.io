...
=E_{H}[log(P(\varphi^1;\mathbf{β_1})P(\varphi_2;\mathbf{β_2})...P(\varphi_K;\mathbf{β_K}))]\\
=E_{H}[logP(\varphi^1;\mathbf{β_1})+logP(\varphi_2;\mathbf{β_2})+...+logP(\varphi_K;\mathbf{β_K})]\\
$$ 
...
$$
E_{H}[log P(W;Z,\varphi)]=E_{Q}[log P(W_1,W_2...W^M;Z_1,Z_2...Z_M,\varphi_1,\varphi_2...\varphi_K)]\\
...
E_{H}[logQ(\varphi;\phi)]=E_{H}[logQ(\varphi^1;\phi^1)+logQ(\varphi^2;\phi^2)+...+logQ(\varphi^K;\phi^K)]\\
E_{H}[logQ(Z;\pi)]=E_{H}[logQ(Z^1;\pi^1)+logQ(,Z^2;\pi^2)+...+logQ(Z^M;\pi^M)]\\

$$   

只关注文章d则：
$$
E_{H}[logP(\theta_d;\mathbf{α^d})]=[log(\frac{\Gamma(\sum_{i=1}^K \alpha_{i})}{\prod_{i=1}^{K}\Gamma(\alpha_{i})} \prod_{i=1}^{K}\theta^{d,i}^{\alpha_{i}-1})]
