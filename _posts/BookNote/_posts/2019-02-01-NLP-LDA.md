...
$$\theta_{d}$$是Dir分布的变量，$$\mathbf{α_{(d)}}$$是参数
...
$$
E_{H}[logP(\theta;\mathbf{α})]=E_{Q}[logP(\theta_1,\theta_2...\theta_M;\mathbf{α_1},\mathbf{α_2}...\mathbf{α_M})]\\
...
$$
E_{H}[logP(Z;\theta)]=E_{Q}[logP(Z_1,Z_2...Z_M;\theta_1,\theta_2...\theta_M)]\\
...
$$
E_{H}[logP(Z_d;\theta_d)]=E_{H}[\sum_{t=1}^{N_{d}}logP(z_{d,t};\theta_d)]\\
...
$$
E_{H}[logP(W_d;\varphi_{Z_d})]=E_{H}[\sum_{t=1}^N logP(W_{d,t};\varphi_{Z_d})]\\
...
$$
E_{H}[logP(\varphi_k;\mathbf{β_k})]=log\Gamma(\sum_{v=1}^V \beta_{k,v}) -\sum_{v=1}^{V}log\Gamma(\beta_{k,v}) + \sum_{v=1}^{V}(\beta_{k,v}-1)(\Psi(\phi_{k,v})-\Psi(\sum_{v=1}^V \phi_{k,v}))
$$
...
\gamma_{d,k}=\alpha_{d,k}+\sum_{t=1}^{N_{j}}\pi_{d,t,k}\\
