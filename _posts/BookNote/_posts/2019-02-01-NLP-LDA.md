...
$$
P(Z_{(m,n)}=k, \mathbf{Z_{-(m,n)}},W;\mathbf{α},\mathbf{β}) \propto (n_{m,(\cdot)}^{k,-(m,n)}+\alpha_{k})\frac{n_{(\cdot),(m,n)}^{k,-(m,n)}+\beta_{(m,n)}}{\sum_{v=1}^V n_{(\cdot),v}^{k,-(m,n)}+\beta_{v}}  
$$
...
\varphi_{k,v} = frac {\sum_{d=1}^{M} \parallel \theta_{d}==k \parallel \parallelw_{d,t}==v\parallel }{\sum_{d=1}^{M} \parallel \theta_{d}==k \parallel N_{d}}  
$$   
