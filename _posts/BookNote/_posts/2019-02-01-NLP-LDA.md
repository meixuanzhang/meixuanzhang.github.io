...
令$$C(H)=\frac{P(W \mid H;\mathbf{α^{(i)}},\mathbf{β^ {(i)}})P(H;\mathbf{α},\mathbf{β})}{P(H \mid W;\mathbf{α},\mathbf{β})}$$则：  
...
=\sum_{H}Q(H;\gamma,\pi,\phi)log\frac{P(W, H;\mathbf{α},\mathbf{β})}{Q(H;\gamma,\pi,\phi)}\\
$$  
