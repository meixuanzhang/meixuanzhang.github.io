...
Z^*=\mathop{\arg\max}_{z_{1}..z_{n}}P(Z \mid Y;\theta)\\
...
由于$$P(Z_{1}\mid Y;\theta)=P(z_{11},z{12}..z{1,k}\mid Y;\theta)，z_{11},z{12}..z{1,k}$$是0,1向量，    
$$P(Z_{j}\mid Y;\theta)=P(z_{jk},=1\mid Y;\theta),k=1,2,...K$$,对于每个样本j,哪个k取1会使概率最大则取1，其他元素取0。
由此计算每一个$$z_{jk}$$  
