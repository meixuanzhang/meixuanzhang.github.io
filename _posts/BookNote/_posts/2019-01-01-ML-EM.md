...
$$\theta^{(i+1)}={argmax}\limits_{\theta}(L(\theta^{(i)})+(\sum\limits_{z}P(Z|Y;\theta^{(i)})) log\frac{P(Y|Z; \theta)P(Z;\theta)}{P(Z|Y;\theta^{(i)})P(Y;\theta^{(i)})})$$  

$$={argmax}\limits_{\theta}((\sum\limits_{z}P(Z|Y;\theta^{(i)})) logP(Y|Z; \theta)P(Z;\theta))$$  

$$={argmax}\limits_{\theta}((\sum\limits_{z}P(Z|Y;\theta^{(i)})) logP(Y,Z; \theta))$$  

$$={argmax}\limits_{\theta}E_{z}(logP(Y,Z)|Y;\theta)$$  
