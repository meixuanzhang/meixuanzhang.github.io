...
令$$P(Z|Y;\theta^{(i)})=P_{Z_{k}}$$ Z是onehot 向量，变量取值只有k种不同，则：
$$
Q = \sum\limits_{k=1}^{K} P_{Z_{k}} \lbrace\sum\limits_{k=1}^{K} n_{k} log\alpha_{k}+\sum\limits_{j=1}^N{z_{jk}}[log(\frac{1}{\sqrt{2\pi}})-log\sigma_{k}-\frac{1}{\sqrt{2\sigma_{k}^2}}(Y_{j}-\mu_{k})^2]\rbrace
$$
