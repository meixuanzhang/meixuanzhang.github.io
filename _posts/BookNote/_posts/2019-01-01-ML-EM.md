...
+ 注意这里的 $$\theta$$ 是我们待估计的参数，是常数，如果是随机变量公式应写成$$P(Y| \theta)$$
...
令$$C(Z）=\frac{P(Y|Z; \theta)P(Z;\theta)} {P(Z|Y;\theta^{(i)})}$$    

$$log(\sum\limits_{z}P(Z|Y;\theta^{(i)})\frac{P(Y|Z; \theta)P(Z;\theta)}{P(Z|Y;\theta^{(i)})}) = log( \sum\limits_{z}P(Z|Y;\theta^{(i)})C(Z)) $$  

$$(\sum\limits_{z}P(Z|Y;\theta^{(i)})) log\frac{P(Y|Z; \theta)P(Z;\theta)}{P(Z|Y;\theta^{(i)})} = (\sum\limits_{z}P(Z|Y;\theta^{(i)})) logC(Z)$$  
