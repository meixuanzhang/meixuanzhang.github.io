...
由于$$P(Z_{1}\mid Y;\theta)=P(z_{11},z_{12}..z{1,k}\mid Y;\theta)，z_{11},z{12}..z{1k}$$是0,1向量，    
$$P(Z_{1}\mid Y;\theta)=P(z_{1k},=1\mid Y;\theta),k=1,2,...K$$,对于每个样本1,哪个k取1会使概率最大则取1，其他元素取0。  
由此类推计算每个样本j,哪个k取1会使概率最大则取1，其他元素取0。确定每个$$z_{jk}$$的值  
