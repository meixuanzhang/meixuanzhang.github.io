...
$$P(Z_{1}\mid Y;\theta)=P(z_{1k},=1\mid Y;\theta),k=1,2,...K$$,对于样本1,哪个元素取$$z_{11},z_{12}..z_{1k}$$取1会使概率最大则取1，其他元素取0。  
由此类推计算每个样本j,哪个取1会使概率最大则取1，其他元素取0。确定每个$$z_{jk}$$的值。   

$$N_{k}=\sum\limits_{j=1}^{N}\mid z_{jk}\mid=1   ,k=1,2...K$$属于k类的样本个数   
...
$$\mu_{k}=\frac{\sum\limits_{j:z_{jk}=1} Y_{j}}{N_{k}} ,k=1,2...K$$  分子是样本为k类的$$Y_{j}$$的和
