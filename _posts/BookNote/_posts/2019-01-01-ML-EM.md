...
式中，$$n_k=\sum\limits_{j=1}^N z_{j1},\sum\limits_{k=1}^K n_k=N$$

完全数据的对数似然函数为  

$$logP(Y,Z;\theta)=\sum\limits_{k=1}^K n_k\alpha_{k}+\sum\limits_{j=1}^N{z_{jk}}[log(\frac{1}{\sqrt{2\pi})-log\sigma_{k}-\frac{1}{\sqrt{2\sigma_{k}^2}(Y_{j}-/mu_{k})^2]$$  
