...
$$
logP(Y,Z; \theta)=logP(Y,Z_{z_{jk}=1}; \theta)
=logP(Y_{1}..Y_{n},z_{1k}=1..z_{nk}=1; \theta)
=logP(Y_{1},z_{1k}=1); \theta)..P(Y_{n},z_{nk}=1; \theta)
$$

令:  

$$P_{jk}=P(z_{jk=1} |Y_{j};\theta^{(i)})$$  
...
$$\mu_{k}=\frac{\sum\limits_{j=1}^N P_{jk} Y_{j}}{\sum\limits_{j=1}^N P_{jk}} ,k=1,2...K$$  

$$\sigma_{k}^2=\frac{\sum\limits_{j=1}^N P_{jk}(Y_{j}-\mu_{k})^2}{\sum\limits_{j=1}^N P_{jk}} ,k=1,2...K$$  

$$\alpha_{k}=\frac{\sum\limits_{j=1}^N P_{jk} }{N} ,k=1,2...K\\$$  
...
为了确定Q，需要知道$$z_{jk}$$的值，实际就是哪些$$z_{jk}=1$$    
