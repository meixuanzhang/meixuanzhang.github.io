...
# EM算法的推导
Nocation:  
$$Y_{j}$$：可观测到的变量,这里的$$Y_{j}=(y^{1}_{j},y^{2}_{j},y^{3}_{j}..y^{n}_{j})$$是多维随机变量。  
Z：隐变量，也是多维随机变量$$Z=(z^{1},z^{2},z^{3}..z^{n})$$  
$$\theta$$:参数  

**概率模型假设：**  

$$P(Y_{j};\theta)=\sum\limits_{z}P(Y_{j},Z; \theta)=\sum\limits_{z}P(Z;\theta)P(Y_{j}|Z; \theta)$$  

+ 公式涉及了贝叶斯定理   
+ 含有隐变量的概率模型我们将Y_{j}，Z称为完全数据，只有Y_{j}称为不完全数据    
...
=E_{z}\lbrace\sum\limits_{k=1}^{K}\sum\limits_{j=1}^N{z_{jk}} log\alpha_{k}+\sum\limits_{k=1}^{K}\sum\limits_{j=1}^N{z_{jk}}[log(\frac{1}{\sqrt{2\pi}})-log\sigma_{k}-\frac{1}{\sqrt{2\sigma_{k}^2}}(Y_{j}-\mu_{k})^2]\rbrace
$$    
...
$$b=\sum\limits_{j=1}^N P_{jk}{z_{jk}}$$    

则：  

$$
Q=\sum\limits_{k=1}^{K} b log\alpha_{k}+ \sum\limits_{k=1}^{K}\sum\limits_{j=1}^NP_{jk}{z_{jk}}[log(\frac{1}{\sqrt{2\pi}})-log\sigma_{k}-\frac{1}{\sqrt{2\sigma_{k}^2}}(Y_{j}-\mu_{k})^2]
$$
