...
$$P(Z_{1}\mid Y;\theta)=P(z_{1k},=1\mid Y;\theta),k=1,2,...K$$,对于样本1,哪个元素$$(z_{11},z_{12}..z_{1k})$$取1会使概率最大则取1，其他元素取0。  
...
这里我们使用H作为隐变量，是一维变量，隐变量取值fany{}
$$\sum\limits_{j=1}^N (Y_{j}-\mu^{z_{jk}=1})^2$$
