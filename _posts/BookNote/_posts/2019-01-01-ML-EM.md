...
$$\theta_{k}=(\mu_{k},\sigma_{k}^2), n\theta=(\alpha_{1},\alpha_{2},...\alpha_{k};\theta_{1},\theta_{2}...\theta_{k})$$  
我们用EM算法估计高斯混合模型的参数$$\theta$$  

1、明确隐变量，写出完全数据的对数似然函数   
假设观测数据$$Y_{j},j=1,2,...N$$(j是样本编码),产生是依据概率$$\alpha_{k}$$选择第k个高斯分布模型 ，  
然后依第k个分模型的概率分布$$\phi(Y;\theta_{k})$$生成观测数据$$Y_{j}$$。  
这时观测数据Y_{j}是已知的，但其来自第k个分模型的数据是未知的，以隐变量Z表示    
$$Z=(z_{j1},z_{j2}...z_{jk})$$  

z_{jk} =1 ,第j个观测来自第k个分模型
z_{jk} =0，否则
