...
# EM算法的推导
Nocation:  
$$Y_{j}$$：可观测到的变量,这里的$$Y_{j}=(y_{j1},y_{j2},y_{j3}..y^_{js})$$是s维随机变量。  
$$Z_{j}$$：隐变量，是k维随机变量$$Z=(z_{j1},z^{j2},z^{j3}..z^{jk})$$  
$$\theta$$:参数  

**概率模型假设：**  

$$P(Y_{j};\theta)=\sum\limits_{z}P(Y_{j},Z_{j}; \theta)=\sum\limits_{z}P(Z_{j};\theta)P(Y_{j}|Z_{j}; \theta)$$  
...
$$log(\sum\limits_{z_{1}..z_{n}}P(Y|Z;\theta)P(Z;\theta))=log(\sum\limits_{z}P(Z|Y;\theta^{(i)})\frac{P(Y|Z; \theta)P(Z;\theta)}{P(Z|Y;\theta^{(i)})})$$  
