...
$$<L(\theta;\mathbf{x},\mathbf{y})>_{q} = logp(\mathbf{x},\mathbf{y};\theta)\\
=\sum_{n}(<y_{n,1}^i>_{p(y_{n,1}\mid x_{n})}log\pi_{i})+ \sum_{n}\sum_{t=2}^{T}(<y_{n,t-1}^i,y_{n,t}^j>_{p(y_{n,t-1},y_{n,t}\mid x_{n})}loga_{i,j})+\sum_{n}\sum_{t=1}^{T}(x_{n,t}^k<y_{n,t}^i>_{p(y_{n,t}\mid x_{n})}logb_{i,k})
$$  


+ Expectation Step：固定$$\theta$$并计算边缘后验  

$$\gamma_{t}^n=<y_{t}^n>=p(y_{t}=i\mid \mathbf{x};\theta),\\
\xi_{t}^n=<y_{t}^n,y_{t-1}^n>=p(y_{t}=i,y_{t-1}=j\mid \mathbf{x};\theta)$$


+ Maximization Step:通过MLE更新$$\theta$$   

$$
\pi^{ML} = \frac{\sum_{n}\gamma_{1}^n}{N}\\
\mathbf{A}^{ML}=\frac{\sum_{n}\sum_{t=2}^{T}\xi_{t}^n}{\sum_{n}\sum_{t}^{T-1}\gamma_{t}^n}\\
b^{ML}=\frac{\sum_{n}\sum_{t}^{T}\gamma_{t}^n x_{t}^n}{\sum_{n}\sum_{t}^{T-1}\gamma_{t}^n}
$$
