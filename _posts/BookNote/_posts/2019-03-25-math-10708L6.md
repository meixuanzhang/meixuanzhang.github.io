...
+ E-step:$$q^{t+1} = argmax_{q}F(q;\theta^t)$$  
...
$$\theta^{t+1} = argmax_{\theta}F(q^t,\theta)=argmax_{\theta}<L(\theta;x,z)>_{q^t}$$  
...
=sum_{n}(<y_{n,1}^i>_{p(y_{n,1}\mid x_{n})}log\pi_{i})+ sum_{n}sum_{t=2}^{T}(<y_{n,t-1}^i,y_{n,t}^j>_{p(y_{n,t-1},y_{n,t}\mid x_{n})}loga_{i,j})+sum_{n}sum_{t=1}^{T}(<y_{n,t}^i>_{p(y_{n,t}\mid x_{n})}logb_{i,k})
$$  
