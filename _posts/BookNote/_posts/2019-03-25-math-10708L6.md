...
与完全观测的GMs相比，学习部分观测图模型或潜在变量模型的参数要困难得多。这是因为未观测变量或隐变量通过边际化(marginalization)与观测变量联系在一起，我们不能再将最大似然函数分解成独立的条件概率。

EM算法为隐变量模型的似然函数最大化提供了一种途径。当原始问题可以分解为两个步骤时，我们可以找到最大似然估计参数：（1）利用我们的观测数据和当前参数提供的最佳猜测(后验)估计或“填充”未观测到的潜在变量，（2）基于此猜测更新参数：
