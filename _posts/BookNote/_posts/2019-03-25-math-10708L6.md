...
$$p(x_{n})=\sum_{z}P(x_{n}\mid z)P(z)=\sum_{k=1}^K N(x_{n}\mid \mu_{k},\Sigma_{k})\pi_{k}$$   
...
$$p(x_{n}\mid z_{n}^k =1)=p(x_{n}\mid \mu_{k},\Sigma_{k})\\
...
=\sum_{z_{n}} \prod_{k}(\pi_{k})^{z_{n}^k} N(x\mid \mu_{k},\Sigma_{k})^{z_{n}^k}\\
=\sum_{k=1}^K N(x_{n}\mid \mu_{k},\Sigma_{k})\pi_{k} $$
