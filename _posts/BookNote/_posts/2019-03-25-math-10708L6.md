...
$$p(z^k=1\mid x,y;\theta)=\frac{p(z^k=1\mid x)p_{k}(y\mid x;\theta_{k},\sigma_{k}^2)}{\sum_{j}p(z^j=1\mid x)p_{j}(y\mid x;\theta_{j},\sigma_{j}^2)}$$

$$p(z^k=1\mid x)p_{k}(y\mid x;\theta_{k},\sigma_{k}^2) = \frac{p(z^k=1,x)}{p_{k}(x)}\frac{p_{k}(x,y)}{p_{k}(x)}$$  

$$p(z^k=1\mid x,y;\theta)=p(z^k=1,x)\frac{p_{k}(x,y)}{p_{k}(x)} \sum_{j}p(z^j=1,x)\frac{p_{j}(x,y)}{p_{j}(x)}$$
...
M-step计算每个专家的线性回归，每个数据点由专家的责任加权
