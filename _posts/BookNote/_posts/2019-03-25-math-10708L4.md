...
1、将分配给每个团$$C_{i}$$的因子相乘，产生初始势函数  

$$\psi_{i}(C_{i})=\prod \psi$$

2、选择含感兴趣变量的根$$C_{r}$$作为团。

3、将图形向上指向根。这定义了树上操作的部分顺序。  

4、自下而上地传递消息（收集证据阶段）：按照从叶到根的拓扑顺序进行计算和存储  

$$\delta_{i \to j}=\sum_{C_{i}-S_{i,j}}\psi_{i} \cdot \prod_{k \in Pred(i)}\delta_{k \to i}$$


5、自上而下分发消息（分发证据阶段）：针对每个团$$C_{i}$$:

$$\beta_{i} = \psi_{r} \cdot \prod_{k \in Pred(i)}\delta_{k \to r}$$

在第4步之后，我们可以通过将传入消息与C自身的潜在势力相乘来获得根节点P的边际： 

$$P(C_{r})= \psi_{r} \cdot \prod_{k \in Pred(i)}\delta_{k \to r}$$
