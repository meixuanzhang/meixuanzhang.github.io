...
如果先去掉质心，很容易看出导出图的宽度等于N-1，其中N是节点总数。但是，如果我们最后删除质心，则很容易看到导出图是原始星图，宽度仅为1。使用这种消除顺序可以使variable elimination 非常有效。  
...
很难找到一个最优的消去顺序。实际上，ising模型的tree width比n大，所以即使找到一个最优的消去顺序，VE算法仍然是指数的。   
