...
在图$$K=(X,\varepsilon)$$中，若$$x_{i}\to x_{j}$$,则$$x_{i}$$是$$x_{j}$$子结点(child),而$$x_{j}$$是$$x_{i}$$父节点(parent)。若$$x_{i}-x_{j}$$，则$$x_{i}$$是$$x_{j}$$的邻节点(neighbor)反之亦然。只要$$x_{i}\rightleftharpoons x_{j} \in \varepsilon$$,我们则称$$x_{i},x_{j}$$相邻(adjacent)，则表示$$x_{i},x_{j}$$由某条边(有向或无向)连接。  
