---
layout: post
title: Probabilistic Graphical Models :Representation 1
date:   2019-03-23
categories: 概率图模型
---
...
在图$$K=(X,\varepsilon)$$中，如果对于每一个$$i=1,2..,k-1$$有$$x_{i}\to x_{i+1}$$或$$x_{i}-x_{i+1}$$,则$$x_{1}..x_{k}$$形成了一条路径(path),如果至少有一个i满足$$x_{i}\to x_{i+1}$$，我们则说这条路径是有向的。  

对于每一个$$i=1,2..,k-1$$有$$x_{i}rightleftharpoons x_{i+1}$$，那么$$x_{1}..x_{k}$$在图$$K=(X,\varepsilon)$$中形成一条迹(trail)。
上图中A,C,D,E,I是一条路径，也是一条迹，A,C,F,G,D是一条迹，但不是一条路径。

对于图中任意节点$$x_{i},x_{j}$$之间存在一条迹，那么该图是连通图。
...
有向图$$g={X,\varepsilon}$$，如果节点$$x_{1}...x_{n}$$序列中，只要$$x_{i}\to x_{j}\in \varepsilon $$,就有$$i<j$$,将序列称为拓扑序(topological ordering)  
